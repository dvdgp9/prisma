...
-- Note: Password encryption will be handled by PHP (openssl_encrypt)
-- Update EBone company with SMTP credentials (password will be encrypted by admin panel)
UPDATE `companies` 
SET 
...
-- Manually set encrypted password through admin panel or run this:
-- UPDATE companies SET smtp_password = '<encrypted_value_from_php>' WHERE LOWER(name) = 'ebone';
